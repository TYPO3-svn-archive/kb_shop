 ...
 *
 *
 *   58: class tx_kbshop_tceFunc_selectTreeView extends t3lib_treeview
 *   70:     function wrapTitle($title,$v)
 *
 *
 *   89: class tx_kbshop_treeview
...
require_once(PATH_t3lib.'class.t3lib_treeview.php');
	 ...
	 *
	 */
class tx_kbshop_tceFunc_selectTreeView extends t3lib_treeview {
	...
	function wrapTitle($title,$v)	{
		if($v['uid']>0) {
			if (in_array($v['uid'],$this->TCEforms_nonSelectableItemsArray)) {
				...
				$aOnClick = 'setFormValueFromBrowseWin(\''.$this->TCEforms_itemFormElName.'\','.$v['uid'].',\''.$title.'\');'.implode('', is_array($this->fieldChangeFunc)?$this->fieldChangeFunc:array()).'; return false;';
	...
	/**
	 * this class displays a tree selector with nested kb_shop categories.
	 *
	 */
class tx_kbshop_treeview {
	...
	function displayCategoryTree($PA, $fobj)    {
		...
		$pid = $row['pid'];
		if ($pid < 0)	{
			$pidRec = tx_kbshop_abstract::getPage(abs($pid));
			$pid = $pidRec['uid'];
		}

		$AddWhere = str_replace('###PAGE_TSCONFIG_ID###', intval($TSconfig[$field]['PAGE_TSCONFIG_ID']), $PA['fieldConf']['config']['foreign_table_where']);
		$AddWhere= str_replace('###PAGE_TSCONFIG_IDLIST###', $GLOBALS['TYPO3_DB']->cleanIntList($TSconfig[$field]['PAGE_TSCONFIG_IDLIST']), $AddWhere);
		$AddWhere = str_replace('###PAGE_TSCONFIG_STR###', $GLOBALS['TYPO3_DB']->quoteStr($TSconfig[$field]['PAGE_TSCONFIG_STR'], $foreign_table), $AddWhere);

		$this->config = $config = $PA['fieldConf']['config'];
		$this->labelField = $TCA[$config['foreign_table']]['ctrl']['label'];
		$this->descField = $TCA[$config['foreign_table']]['ctrl']['description_field'];
		$this->foreignTable = $config['foreign_table'];
		if (!$this->descField)	{
			$this->descField = $this->labelField;
		}
		// it seems TCE has a bug and do not work correctly with '1'
//		$config['maxitems'] = ($config['maxitems']<0) ? 1 : $config['maxitems'];
		...
		foreach($selItems as $tk => $p)	{
			if ($this->foreignTable===$this->confObj->categoriesTable)	{
				$cUid = intval($p[1]);
				while ($cUid)	{
					$cat = tx_kbshop_abstract::getRecord($this->confObj->categoriesTable, $cUid);
					$cUid = $cat['parent'];
				}
				$tkey = tx_kbshop_misc::getKey($cat);
				if ($table!=$this->confObj->entriesTablePrefix.$tkey)	{
					unset($selItems[$tk]);
					continue;
				}
			}
		...
		if ($maxitems<1 AND !$config['treeView'])	{
			...
			if ($row['sys_language_uid'] && $row['l18n_parent'] && !$config['langChildren']) { // the current record is a translation of another record
				if ($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['kb_shop']) { // get kb_shop extConf array
					$confArr = unserialize($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['kb_shop']);
				}
				if ($confArr['useStoragePid']) {
					$TSconfig = t3lib_BEfunc::getTCEFORM_TSconfig($table,$row);
					$storagePid = $TSconfig['_STORAGE_PID']?$TSconfig['_STORAGE_PID']:0;
					$SPaddWhere = ' AND '.$config['foreign_table'].'.pid IN (' . $storagePid . ')';
				}
				$errorMsg = array();
...
//				print_r($config);
				if ($config['MM'])	{
					$catres = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query ($config['foreign_table'].'.uid,'.$config['foreign_table'].'.'.$this->labelField.','.$config['MM'].'.sorting AS mmsorting', $table, $config['MM'], $config['foreign_table'], ' AND '.$config['MM'].'.uid_local='.$row['l18n_parent'].$SPaddWhere.$AddWhere,'', 'mmsorting');
				} else	{
					$catres = $GLOBALS['TYPO3_DB']->exec_SELECTquery ($config['foreign_table'].'.uid,'.$config['foreign_table'].'.'.$this->labelField, $config['foreign_table'], $config['foreign_table'].'.uid IN ('.$row[$this->field].') '.$SPaddWhere.$AddWhere,'', ' FIELD('.$config['foreign_table'].'.uid, '.$row[$this->field].')');
				}
				/*
				if ($config['MM'])	{
				} else	{
					$ores = $GLOBALS['TYPO3_DB']->exec_SELECTquery($table.'.'.$this->field, $table, 'uid='.$row['l18n_parent']);
					$orec = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($orec);
					if (is_array($orec)) {
						$catres = $GLOBALS['TYPO3_DB']->exec_SELECTquery ($config['foreign_table'].'.uid,'.$config['foreign_table'].'.'.$this->labelField, $config['foreign_table'], $config['foreign_table'].'.uid IN ('.$orec[$this->field].') '.$SPaddWhere);
					}
				}
				*/
				$categories = array();
				...
				while ($catrow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($catres)) {
					if(in_array($catrow['uid'],$notAllowedItems)) {
						$categories[$catrow['uid']] = $NACats[] = '<p style="padding:0px;color:red;font-weight:bold;">- '.$catrow[$this->labelField].' <span class="typo3-dimmed"><em>['.$catrow['uid'].']</em></span></p>';
						$na = true;
					} else {
						$categories[$catrow['uid']] = '<p style="padding:0px;">- '.$catrow[$this->labelField].' <span class="typo3-dimmed"><em>['.$catrow['uid'].']</em></span></p>';
					}
				}
				...
				if($config['treeView'] AND $config['foreign_table']) {
					...
					if($config['treeViewClass'] AND is_object($treeViewObj = &t3lib_div::getUserObj($config['treeViewClass'],'user_',false)))      {
						...
						$treeViewObj = t3lib_div::makeInstance('tx_kbshop_tceFunc_selectTreeView');
					}
					$treeViewObj->labelField = $this->labelField;
					$treeViewObj->descField = $this->descField;
					...
					$treeViewObj->clause = $where.$AddWhere;
...
//							&& $GLOBALS['BE_USER']->jsConfirmation(1)
						...
						$alertMsgOnChange = 'if (confirm('.$GLOBALS['LANG']->JScharCode($GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.xml:mess.onChangeAlert')).') && TBE_EDITOR_checkSubmit(1)){ TBE_EDITOR_submitForm() };';
						$PA['fieldChangeFunc']['alert'] = $alertMsgOnChange;
					} else {
						$alertMsgOnChange = '';
					}
					$treeViewObj->fieldChangeFunc = $PA['fieldChangeFunc'];

					if ($this->descField!=$this->labelField)	{
						$treeViewObj->fieldArray[] = $this->descField;
					}
					...
					if (is_array($notAllowedItems) && $notAllowedItems[0]) {
						...
						// get default items
					$defItems = array();
					if (is_array($config['items']) && $table == 'tt_content' && $row['CType']=='list' && $field == 'pi_flexform')	{
						reset ($config['items']);
					...
					$treeIds = $treeViewObj->ids;
					if ($tabCatUid)	{
						$treeIds[] = $tabCatUid;
					}
					$errorMsg = $this->findRecursiveCategories($PA,$row,$table,$storagePid,$treeIds) ;
				...
				foreach($fromArray as $tk => $tv) {
					...
					if (in_array($tvP[0],$removeItems) && !$PA['fieldTSConfig']['disableNoMatchingValueElement'])	{
						...
						if (!strlen($tvP[1]))	{
							$rec = t3lib_BEfunc::getRecordWSOL($this->config['foreign_table'], intval($tvP[0]));
							if (is_array($rec))	{
								$tvP[1] = rawurlencode(t3lib_BEfunc::getRecordTitle($this->config['foreign_table'], $rec));
							} else	{
								$tvP[1] = intval($tvP[0]);
							}
						} 
				...
				// Wizards:
				$altItem = '<input type="hidden" name="'.$PA['itemFormElName'].'" value="'.htmlspecialchars($PA['itemFormElValue']).'" onchange="'.$alertMsgOnChange.'" />';
	 ...
	 * @see tx_kbshop_tceFunc_selectTreeView::wrapTitle()
	 */
	function getNotAllowedItems($PA,$SPaddWhere) {
		...
		$allowedItemsList=$GLOBALS['BE_USER']->getTSConfigVal('plugin.kb_shop.perms.'.$fTable.'.allowedItems');
	...
	function findRecursiveCategories ($PA,$row,$table,$storagePid,$treeIds) {
		$errorMsg = array();
		if ($table == 'tt_content' && $row['CType']=='list') { // = tt_content element which inserts plugin kb_shop
			...
			if ($table == $this->config['foreign_table'] && $row['pid'] == $storagePid && intval($row['uid']) && !in_array($row['uid'],$treeIds))	{ // if the selected category is not empty and not in the array of tree-uids it seems to be part of a chain of recursive categories
			...
			if ($table != $this->config['foreign_table'] && $row[$this->field]) { // find recursive categories in the kb_shop db-record
				$rcList = $this->compareCategoryVals ($treeIds,$row[$this->field]);
			}
			// in case of localized records this doesn't work
			if ($storagePid && $row['pid'] != $storagePid && $table == $this->config['foreign_table']) { // if a storagePid is defined but the current category is not stored in storagePid
		...
		if (strlen($rcList)) {
			...
			if ($table != $this->config['foreign_table']) {
	...
	/**
	 * This functions displays the title field of a shop record and checks if the record has categories assigned that are not allowed for the current BE user.
	...
	function displayTypeFieldCheckCategories(&$PA, $fobj)    {
		...
		if ($GLOBALS['BE_USER']->getTSConfigVal('options.useListOfAllowedItems') && !$GLOBALS['BE_USER']->isAdmin()) {
			...
			if ($notAllowedItems[0]) {
				...
				$catres = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query ($config['foreign_table'].'.uid,'.$config['foreign_table'].'.'.$this->labelField.','.$config['MM'].'.sorting AS mmsorting', $table, $config['MM'], $config['foreign_table'], ' AND '.$config['MM'].'.uid_local='.$uidField.$SPaddWhere,'', 'mmsorting');
				$NACats = array();
				while ($catrow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($catres)) {
					if($catrow['uid'] && $notAllowedItems[0] && in_array($catrow['uid'],$notAllowedItems)) {
						$NACats[]= '<p style="padding:0px;color:red;font-weight:bold;">- '.$catrow[$this->labelField].' <span class="typo3-dimmed"><em>['.$catrow['uid'].']</em></span></p>';
					}
				}
				if($NACats[0]) {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kb_shop/class.tx_kbshop_treeview.php'])    {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kb_shop/class.tx_kbshop_treeview.php']);
}
?>
